...
client: Optional[TelegramClient] = None # Initialize as None, will be set in run_bot
...
async def run_bot():
    ...
    global client # Make client global so handlers and workers can access it
    client = TelegramClient(
        'bot_24_7_session',
        API_ID,
        API_HASH,
        connection_retries=10,  # More retries for better reliability
        auto_reconnect=True,    # Always try to reconnect
        retry_delay=1,          # Short delay between retries
        timeout=DOWNLOAD_TIMEOUT # Apply timeout to client operations
    )
